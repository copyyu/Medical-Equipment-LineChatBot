...
func SetupNotificationRoutes(app *fiber.App, notificationHandler *handlers.NotificationHandler, adminUsecase usecase.AdminUsecase) {
	...
	// Public: the Excel export link is embedded as an "Export" button in the LINE
	// Flex alerts and is opened in the user's browser without a bearer token, so
	// it cannot sit behind AuthMiddleware. (Access is intended to be gated by a
	// signed URL — see follow-up.)
